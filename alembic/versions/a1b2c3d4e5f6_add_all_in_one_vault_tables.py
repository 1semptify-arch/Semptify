...
down_revision: str | None = "20250424_add_search_indexes"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("event_time", sa.DateTime(timezone=True), nullable=False, comment="Factual time of event occurrence"),
        sa.Column(
            "record_time", sa.DateTime(timezone=True), nullable=False, comment="When evidence was created/recorded"
        ),
        sa.Column(
            "semptify_entry_time", sa.DateTime(timezone=True), nullable=False, comment="When added to Semptify system"
        ),
    ...
    # ==========================================================================
    op.create_index("idx_vault_metadata_gin", "vault_items", [sa.text("metadata")], postgresql_using="gin")
    op.create_index("idx_vault_location_gin", "vault_items", [sa.text("location_data")], postgresql_using="gin")
    op.create_index("idx_vault_tags_gin", "vault_items", [sa.text("tags")], postgresql_using="gin")
    ...
    op.create_index("idx_vault_user_type_time", "vault_items", ["user_id", "item_type", "event_time"])
    op.create_index("idx_vault_incident_time", "vault_items", ["related_incident_id", "event_time"])
    ...
    op.create_index("idx_audit_item_timestamp", "vault_audit_logs", ["item_id", "timestamp"])
