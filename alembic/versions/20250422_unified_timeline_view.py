...
def upgrade():
    ...
    op.create_index("idx_documents_user_uploaded", "documents", ["user_id", sa.text("uploaded_at DESC")])

    op.create_index("idx_timeline_events_user_date", "timeline_events", ["user_id", sa.text("event_date DESC")])

    op.create_index("idx_calendar_events_user_start", "calendar_events", ["user_id", sa.text("start_datetime DESC")])

    # Index on vault_items for the three timestamps
    op.create_index("idx_vault_items_user_event_time", "vault_items", ["user_id", sa.text("event_time DESC")])

    op.create_index("idx_vault_items_user_record_time", "vault_items", ["user_id", sa.text("record_time DESC")])
