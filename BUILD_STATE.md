...
- Untracked leftovers in tree (not shipped): `legal_intel/redacted_property_
...
### Goal
Migrate app/templates/pages/document_delivery_send.html and app/templates/pages/document_delivery_inbox.html to canonical SSOT design tokens and borderless panel elevation.
...
### What changed
- app/templates/pages/document_delivery_send.html
  - Replaced undefined/legacy CSS variables (--space-xxl, --font-size-*, --color-text-*, --color-surface*) with SSOT tokens.
  - Panel now uses var(--zone-surface-2) + var(--elevation-1) (borderless, subtle depth).
  - Inputs keep 1px solid var(--border-color) for affordance.
  - Button uses canonical .btn.btn--primary.
- app/templates/pages/document_delivery_inbox.html
  - Same SSOT token migration.
  - Document list items use var(--zone-surface-1) for row separation without card borders.
  - Status badges use var(--text-inverse) on semantic color fills.
...
### Root cause
The `local_dev` runtime profile disables the embedding model, but `get_embedding_model()`
in `app/modules/context_engine/embedding_model.py` still tried to lazily download/load
`all-MiniLM-L6-v2` on the first guide-page request. With no network or missing cache, the
load hung until the 30s request timeout fired, producing the same 504 symptom everywhere
`get_explanation_for_guide()` is called.

### What changed
- `app/modules/context_engine/embedding_model.py`
  - Imports and checks `get_active_profile().embedding_model` before attempting model load.
  - Returns `None` immediately when the runtime profile has the model off.
  - This keeps `retrieve_explanations()` silent and lets `local_dev` guide pages render in <300ms.
- `app/core/auto_refresh.py`
  - Tracks recent per-user refresh failures to avoid repeated provider calls in the same request.
  - Adds a 30s cooldown so `ensure_valid_token()` fails fast after the first failed attempt.
- `app/modules/storage/router.py`
  - Reduces OAuth token refresh `httpx.AsyncClient` timeout from 30.0s to 5.0s so a single
    unreachable provider does not blow the whole request timeout.
- `app/core/oauth_token_manager.py`
  - Reduces provider refresh timeouts from 10s to 5s for consistency.

### Verification
- `python -m py_compile` on all changed Python files: PASS
- `tools/guardrail_engine.py`: 7/7 PASS
- `pytest tests/module_health/test_law_library.py tests/module_health/test_law_library_page_router.py`: 2 passed
- `GET /gui/know/law-library/get-case` (authenticated dev user): 200 in 272ms
- `GET /gui/record/journal/create` (authenticated dev user): 200 in 136ms
- `GET /api/law-library/case-law/fritz_v_warthen`: 200, returns correct case data

---

## Session — 2026-09-11 — Document Center mobile layout + upload wiring (claude)

### Guardrail Engine Run — 2026-09-11T13:11:32+00:00
...
1. **Page Shell manifest** — `ProductTier.CORE`, `lifecycle="stable"`, used by tenant routes. Gap closed.
...
- `docs/classification-test-fixtures/`:
  ...
  - Real `lease_02.pdf` (copy of `E:\[REDACTED-PATH]\Leases\lese.pdf`) ignored by `.gitignore` so no case data is committed.
...
### New input
Brad provided `E:\[REDACTED-PATH]\Leases\lese.pdf`.
...
- The other PDFs in `E:\[REDACTED-PATH]\Leases\` are clearly the actual lease packet:
...
### Guardrail Engine Run — 2026-08-13T23:33:23

- **contract_route_check**: FAIL — 2 contract/route conformance failure(s).
- **fees_policy_check**: PASS — No exempt_advanced module is reachable by the tenant role.
- **manifest_sync_check**: PASS — Sync orchestrator passed.
- **stub_check**: PASS — No stubs found.
...
### Guardrail Engine Run — 2026-08-13T22:31:51

- **contract_route_check**: FAIL — Contract loader failed: 73 module(s) failed to load.
- **fees_policy_check**: PASS — No exempt_advanced module is reachable by the tenant role.
- **manifest_sync_check**: PASS — Sync orchestrator passed.
- **stub_check**: PASS — No stubs found.
...
### Guardrail Engine Run — 2026-08-13T15:39:47
...
1. **GitHub cleanup** — All repos consolidated under `1semptify-arch/`
   - Archived: `1semptify-arch/Semptify`, `Semptify5.0`, `Semptify0.1`, `semptify-sdk`, `Sempt`
...
**Commits in this push (oldest→newest):** c9b39c54 preflight freshness/canonicity gate (Steps 1a/1b/1c + legal-content bar); 7b937ef7 legacy→master queue auto-promotion + fail-closed drift hook; fbe56c1a queue-sync docs; eab88628 Reset-Local-Postgres default path after D:\ data-dir migration; 3ac2ebf6 [redacted-entity] legal_intel research profile; 3dd9a257 preflight attestation wired into claim path + stale tier/queue docs; 90f28904 `unlimited` defined as usage class with trusted/restricted sub-tiers; 82849a49 preflight "Noticed but not fixed" routes to master intake queue; 4a8b9382 free-pool dispatch order swe-executor > SWE-2.0 > GLM.
...
## Session — 2026-09-15 — Shell migration: public pages + 61 app pages (devin)

**PRs opened (unmerged):**
- #246 `gui/public-pages-shell` — `65fbe6f8` — 8 public legal/policy pages moved off `static/public/*.html` (old chrome) onto `body/public_shell.html`; portal-registry entries added for disclaimer/accessibility/credits/feedback; footer + nav repointed to clean routes; `/public/*.html` + `/{name}.html` legacy URLs 302 to clean routes; 8 static duplicates deleted; `welcome.html` + `semptify-help-standalone.html` kept deliberately (onboarding gate / always-available emergency page).
- #247 `gui/pages-shell-batch1` — `6537253c` — all 61 `app/templates/pages/*` that extended `base.html` now extend `shell_base.html` (drop-in: content block unchanged, sidebar rail free). `template-N` palette overrides dropped — they recolored header/main/footer with the old 3-color sets. Functional page classes (`vault-main`, `dispute-tracker-page`, `eviction-timeline-page`) kept via `{{ super() }}`. Tenant sub-nav (`Home/Quick Capture/Journal/Inbox`) kept in header on 6 pages — fnav conversion is per-page follow-up.

**Audit correction:** the ~67 non-public static HTML files are mostly **live routed pages**, not dead files — `static/admin/*` (~18 FileResponse routes), `static/tenant|advocate|legal/{subpage}` catch-alls, `/manager`, `/office`, mndes router, `/activate-vault`, overlays/docs/ai-helper. They need template migration, not deletion. Truly orphaned: `static/filedored.html`, `static/status.html`, `static/onboarding/{role-select,pick-role}.html`, `static/onboarding/validation/*`, `static/reconnect/index.html` (verify before delete).

**Verified live (local, signed tenant cookie `GU…`):** `/documents` (DC) and `/tenant/journal` render `body.shell` — desktop screenshots clean, rail + quiet header + centered footer, 0 console errors. `/vault` renders shell then its storage-connect JS kicks off OAuth (its own flow, unchanged). Role/workflow-gated routes 302 to `/tenant/start` for fresh users — expected.
