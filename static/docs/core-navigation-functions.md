# Semptify 5 Core Navigation Groups

## Function/Module Organization by Role

---

## 🏠 HOME

### Dashboard, overview, quick access for each role

| Function | Public | Tenant | Advocate | Legal | Manager |
| ---------- | -------- | -------- | ---------- | ------- | --------- |
| Welcome/Landing | ✅ | - | - | - | - |
| Dashboard | - | ✅ | ✅ | ✅ | ✅ |
| Quick Journal Entry | - | ✅ | - | - | - |
| Case Overview | - | - | ✅ | ✅ | - |
| Property Overview | - | - | - | - | ✅ |
| Recent Activity | - | ✅ | ✅ | ✅ | ✅ |
| Notifications | - | ✅ | ✅ | ✅ | ✅ |
| Upcoming Deadlines | - | ✅ | ✅ | ✅ | ✅ |
| Emergency Status | - | ✅ | - | - | - |
| System Health (admin) | ✅ | - | - | - | - |

---

## 📚 LIBRARY

### Research, law, rights, guides, resources

| Function | Public | Tenant | Advocate | Legal | Manager |
| ---------- | -------- | -------- | ---------- | ------- | --------- |
| Rights Guides | ✅ | ✅ | ✅ | ✅ | ✅ |
| State Law Database | ✅ | ✅ | ✅ | ✅ | ✅ |
| Local Ordinances | ✅ | ✅ | ✅ | ✅ | ✅ |
| Legal Research Tools | - | - | ✅ | ✅ | - |
| Case Law Lookup | - | - | - | ✅ | - |
| Form Templates | ✅ | ✅ | ✅ | ✅ | ✅ |
| Sample Letters | - | ✅ | ✅ | ✅ | - |
| Eviction Process Guide | ✅ | ✅ | ✅ | ✅ | ✅ |
| Habitability Checklist | - | ✅ | ✅ | ✅ | ✅ |
| Rent Control Info | ✅ | ✅ | ✅ | ✅ | ✅ |
| Fair Housing Resources | ✅ | ✅ | ✅ | ✅ | ✅ |
| Pro Se Resources | - | ✅ | - | - | - |
| Advocate Training | - | - | ✅ | - | - |
| CLE Materials | - | - | - | ✅ | - |
| Landlord Compliance | - | - | - | - | ✅ |

**Librarian Role:** Research assistance, document request, inter-library loan

---

## 🏢 OFFICE

### Documents, files, communication, paperwork center

| Function | Public | Tenant | Advocate | Legal | Manager |
| ---------- | -------- | -------- | ---------- | ------- | --------- |
| **Document Vault** | - | ✅ | ✅ | ✅ | ✅ |
| - Upload/Store | - | ✅ | ✅ | ✅ | ✅ |
| - Organize | - | ✅ | ✅ | ✅ | ✅ |
| - Search | - | ✅ | ✅ | ✅ | ✅ |
| **Communication Center** | - | ✅ | ✅ | ✅ | ✅ |
| - Inbox | - | ✅ | ✅ | ✅ | ✅ |
| - Compose Message | - | ✅ | ✅ | ✅ | ✅ |
| - Templates | - | ✅ | ✅ | ✅ | ✅ |
| **Notary/Attestation** | - | ✅ | ✅ | ✅ | - |
| **Timeline/Journal** | - | ✅ | ✅ | ✅ | - |
| - Event Entry | - | ✅ | ✅ | ✅ | - |
| - Chronology View | - | ✅ | ✅ | ✅ | - |
| **Packet Builder** | - | ✅ | ✅ | ✅ | ✅ |
| - Complaint Filing | - | ✅ | ✅ | ✅ | - |
| - Response Packet | - | ✅ | ✅ | ✅ | - |
| - Evidence Bundle | - | ✅ | ✅ | ✅ | ✅ |
| **Document Delivery** | - | ✅ | ✅ | ✅ | ✅ |
| - Send to Attorney | - | ✅ | - | - | - |
| - Send to Advocate | - | ✅ | - | - | - |
| - Send to Court | - | - | - | ✅ | - |
| - Send to Landlord | - | ✅ | ✅ | - | ✅ |
| **Overlay Viewer** | - | ✅ | ✅ | ✅ | ✅ |
| **PDF Tools** | - | ✅ | ✅ | ✅ | ✅ |
| - Merge | - | ✅ | ✅ | ✅ | ✅ |
| - Split | - | ✅ | ✅ | ✅ | ✅ |
| - Sign | - | ✅ | ✅ | ✅ | ✅ |
| - Redact | - | - | - | ✅ | ✅ |
| **Document Signer** | - | ✅ | ✅ | ✅ | ✅ |

---

## 🔧 TOOLS

### Utilities, calculators, generators

| Function | Public | Tenant | Advocate | Legal | Manager |
| ---------- | -------- | -------- | ---------- | ------- | --------- |
| **Rent Calculator** | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Proration | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Late Fees | - | ✅ | ✅ | ✅ | ✅ |
| - Security Deposit Interest | - | ✅ | ✅ | ✅ | ✅ |
| **Deadline Calculator** | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Notice Periods | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Court Deadlines | - | - | - | ✅ | - |
| - Appeal Deadlines | - | - | - | ✅ | - |
| **Notice Generator** | - | ✅ | ✅ | ✅ | ✅ |
| - Repair Request | - | ✅ | ✅ | - | - |
| - Rent Withholding | - | ✅ | ✅ | - | - |
| - Lease Violation | - | - | - | - | ✅ |
| - Termination Notice | - | ✅ | ✅ | - | - |
| **Letter Generator** | - | ✅ | ✅ | ✅ | - |
| **Form Filler** | - | ✅ | ✅ | ✅ | ✅ |
| - Court Forms | - | ✅ | ✅ | ✅ | - |
| - Agency Forms | - | ✅ | ✅ | ✅ | - |
| **Inspection Checklist** | - | ✅ | ✅ | - | ✅ |
| **Evidence Organizer** | - | ✅ | ✅ | ✅ | - |
| **Expense Tracker** | - | ✅ | - | - | - |
| **Income/Eligibility** | - | ✅ | ✅ | - | - |
| **Deductible Calculator** | - | - | - | - | ✅ |
| **Cap Rate/Rent Analysis** | - | - | - | - | ✅ |

---

## 🆘 HELP

### Support, contacts, crisis resources, FAQ

| Function | Public | Tenant | Advocate | Legal | Manager |
| ---------- | -------- | -------- | ---------- | ------- | --------- |
| **FAQ** | ✅ | ✅ | ✅ | ✅ | ✅ |
| - General | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Technical | ✅ | ✅ | ✅ | ✅ | ✅ |
| - Billing | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Contact Forms** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Live Chat** | - | ✅ | ✅ | ✅ | ✅ |
| **Crisis Resources** | - | ✅ | ✅ | - | - |
| - Emergency Eviction Hotline | - | ✅ | ✅ | - | - |
| - Legal Aid Directory | ✅ | ✅ | ✅ | - | - |
| - Shelter Resources | - | ✅ | ✅ | - | - |
| - Rent Assistance | - | ✅ | ✅ | - | - |
| - County Housing Resources | ✅ | ✅ | ✅ | - | - |
| **Support Tickets** | - | ✅ | ✅ | ✅ | ✅ |
| **Feedback** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Report Bug** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Feature Request** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Video Tutorials** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **User Guide** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Contact Semptify** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **About Semptify** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Privacy/Security** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Partner Organizations** | ✅ | ✅ | ✅ | - | - |

---

## 🚨 EMERGENCY FUNCTIONS (Tenant State)

These appear in **Tenant** dashboard when emergency mode is active:

- Emergency Eviction Timeline
- 24-Hour Legal Hotline
- Emergency Document Packet
- Same-Day Advocate Assignment
- Crisis Housing Resources
- Emergency Rent Help Lookup

---

## 🔐 SYSTEM/ADMIN FUNCTIONS (Admin role)

- User Management
- Role Assignment
- Storage Provider Config
- System Settings
- Audit Logs
- Health Monitoring
- Page Builder
- Theme Management
- Feature Flags

---

## Summary by Role

| Role | Core Functions | Theme |
| ------ | --------------- | ------- |
| **Public** | Limited Library access, Tools (calculators), Help resources | Ocean |
| **Tenant** | Full access to all 5 cores + Emergency state | Royal |
| **Advocate** | Home caseload, Library research, Office delivery, Tools, Help | Forest |
| **Legal** | Home clients, Library case law, Office privileged docs, Tools, Help | Crimson |
| **Manager** | Home properties, Library compliance, Office tenant docs, Tools, Help | Slate |
| **Admin** | System management across all areas | Ocean |

---

## Next: Build These Pages

### Priority 1 (Core Navigation Landing Pages)

1. `/home.html` - Role-aware dashboard
2. `/library.html` - Research center
3. `/office.html` - Document & communication center
4. `/tools.html` - Utilities hub
5. `/help.html` - Support center

### Priority 2 (Role-Specific Home Pages)

1. `/tenant/index.html` ✅ (exists)
2. `/advocate/index.html` ✅ (exists)
3. `/legal/index.html` ✅ (exists)
4. `/manager/index.html` ✅ (exists)

### Priority 3 (Office Sub-Pages)

- `/office/vault.html` - Document vault
- `/office/inbox.html` - Communication center
- `/office/timeline.html` - Journal/timeline
- `/office/delivery.html` - Send documents
- `/office/signer.html` - Sign PDFs
