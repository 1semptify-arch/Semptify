...
def extract_hearing_date(key_dates: list) -> str | None:
...
def extract_answer_deadline(key_dates: list) -> str | None:
...
def extract_property_address(text: str, key_parties: list = None) -> str | None:
    ...
    if key_parties:
        for party_info in key_parties:
            if isinstance(party_info, dict):
                role = party_info.get("role", "").lower()
                if ("defendant" in role or "tenant" in role) and party_info.get("address"):
                    return party_info["address"]

    return None


def extract_rent_amount(key_amounts: list) -> float | None:
...
def get_document_added_summary(case_data: dict[str, Any], filename: str) -> str:
    ...
    lines = [
        ...
        "   Status: Flagged for re-evaluation",
    ]
