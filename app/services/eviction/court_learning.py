...
class CaseOutcomeRecord:
    ...
    landlord_attorney: str | None = None
...
class DefenseSuccessRate:
    ...
    avg_settlement_savings_cents: int | None = None
    notes: str = ""
...
class LandlordPattern:
    ...
    typical_attorney: str | None = None
    notes: str = ""
...
class CourtLearningEngine:
    ...
    async def get_defense_success_rates(self, county: str = "Dakota", min_cases: int = 5) -> list[DefenseSuccessRate]:
        ...
        for case in self._case_outcomes:
            ...
            for defense in case.defenses_used:
                ...
                if case.outcome == CaseOutcome.WON or case.outcome == CaseOutcome.DISMISSED:
                    stats["wins"] += 1
    ...
    async def get_learning_stats(self) -> dict:
        ...
        return {
            ...
            "counties_covered": list({c.county for c in self._case_outcomes}),
            "date_range": {
...
# Singleton instance
_learning_engine: CourtLearningEngine | None = None
_seeded = False


async def get_learning_engine() -> CourtLearningEngine:
    """Get the court learning engine instance."""
    global _learning_engine, _seeded
    if _learning_engine is None:
        _learning_engine = CourtLearningEngine()
    if not _seeded:
        from app.services.eviction.seed_court_data import seed_learning_engine

        await seed_learning_engine(_learning_engine, num_cases=200)
        _seeded = True
