...
class SignatureProfile:
    ...
    def __post_init__(self):
        if not self.signature_hash:
            self.signature_hash = hashlib.md5(f"{self.signer_name}:{self.signature_text}".encode()).hexdigest()[:12]  # noqa: S324 - non-security dedup hash
...
class HandwritingAnalyzer:
    ...
    async def analyze(
        ...
        reference_signatures: list[SignatureProfile] | None = None,
    ...
    def _extract_handwritten_elements(self, text: str) -> list[HandwrittenElement]:
        ...
        # Extract dates
        for pattern, _pattern_type in self.handwritten_date_patterns:
        ...
        # Extract amounts
        for pattern, _pattern_type in self.handwritten_amount_patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                elements.append(
                    HandwrittenElement(
                        id=make_id("fri"),
        ...
        # Extract initials
        for pattern, _pattern_type in self.initials_patterns:
        ...
        # Extract annotations
        for pattern, _pattern_type in self.annotation_patterns:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                elements.append(
                    HandwrittenElement(
                        id=make_id("fri"),
    ...
    def _detect_flourish(self, name: str) -> bool:
        ...
        return bool(name and name[-1].lower() in "sygnr")
    ...
    def _requires_signature(self, document_type: str | None) -> bool:
