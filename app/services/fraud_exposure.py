...
class FraudFinding:
    ...
    potential_damages: float | None = None
...
class FraudExposureService:
    ...
    async def analyze_fraud(
        ...
        for subsidy in subsidies:
            ...
            if "lihtc" in subsidy_type and subsidy.get("income_violation"):
                finding_id += 1
        ...
        for lender in lenders:
            ...
            if "fha" in lender_type and lender.get("maintenance_issues"):
                finding_id += 1
    ...
    def _generate_recommendations(self, findings: list[FraudFinding]) -> list[str]:
        ...
        fraud_types = {f.fraud_type for f in findings}
        agencies = set()
        for f in findings:
    ...
    def get_report(self, report_id: str) -> FraudReport | None:
    ...
    def check_habitability_fraud(self, violations: list[Any]) -> dict[str, Any]:
        ...
        for violation in violations:
            desc = violation.get("description", str(violation)) if isinstance(violation, dict) else str(violation)
            findings.append(
                {
                    "type": "habitability",
                    "description": desc,
    ...
    def check_mortgage_fraud(self, lender_info: dict[str, Any] | None) -> dict[str, Any]:
        ...
        # FHA violations
        if "fha" in lender_type and lender_info.get("maintenance_issues"):
            findings.append(
                {
        ...
        if ("fannie" in lender_type or "freddie" in lender_type) and lender_info.get("code_violations"):
            findings.append(
                {
    ...
    def get_whistleblower_protections(self, fraud_type: str | None = None) -> dict[str, Any]:
...
# Global instance
_fraud_service: FraudExposureService | None = None
