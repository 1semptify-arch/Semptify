...
from app.core.vault_paths import AUTH_FOLDER, SEMPTIFY_ROOT, TOKEN_FILE as _VAULT_TOKEN_FILE, VAULT_DOCUMENTS
...
class StorageProvider(ABC):
    ...
    async def ensure_semptify_folder(self) -> bool:
        ...
        try:
            ...
            logger.warning("Failed to ensure Semptify folder exists: %s", exc)
            return False
    ...
    async def write_auth_token(self, encrypted_token_data: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to write auth token to storage: %s", exc)
            return False

    async def read_auth_token(self) -> str | None:
        """
        ...
        try:
            ...
            logger.warning("Failed to read auth token from storage: %s", exc)
            return None
    ...
    async def list_documents(
        ...
        try:
            ...
            logger.warning("Failed to list documents in %s: %s", vault_folder, exc)
            return []
