...
async def get_conversation(
    ...
    return await service.get_conversation_messages(conversation_id, before_message_id=before_message_id, limit=limit)
...
async def mark_message_read(
    ...
    success = await service.mark_messages_read(MarkReadRequest(message_ids=[message_id]))
...
async def mark_conversation_read(
    ...
    success = await service.mark_messages_read(MarkReadRequest(conversation_id=conversation_id, mark_all=True))
...
async def reject_document(
    ...
    delivery_result = await delivery_service.reject_document(delivery_id=delivery_id, request=reject_request)

    if not delivery_result.success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=delivery_result.message or "Failed to reject document"
        )
    ...
    await comm_service._save_rejection_record(
        delivery_id=delivery_id, reason=reason, rejected_at=delivery_result.rejected_at
    )
...
async def fill_and_sign_document(
    ...
    signature_request = SignDocumentRequest(
        signature_type=signature_type, signature_value=signature_value, agree_to_terms=agree_to_terms
    )
...
async def upload_attachment(
    ...
    try:
        ...
        attachment = MessageAttachment(
            filename=file.filename, document_id=result.vault_id, file_size=len(content), mime_type=file.content_type
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to upload attachment: {str(e)}"
        )
...
async def get_delivery_conversation(
    ...
    for _conv in conversations.conversations:
...
async def send_typing_indicator(
    ...
    return {"success": True, "conversation_id": conversation_id, "user_id": user.user_id, "is_typing": is_typing}
