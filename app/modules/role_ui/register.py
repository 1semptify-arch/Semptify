...
register_function_group(
    FunctionGroupContract(
        ...
        description=(
            "CANONICAL list of all available roles with metadata. Used by the role selection page during onboarding."
        ),
        inputs=(),
        outputs=("roles",),
        dependencies=("app.routers.role_ui",),
...
register_function_group(
    FunctionGroupContract(
        ...
        description=(
            "CANONICAL pageview tracking stub. Accepts pageview pings silently. Full analytics enabled in ADMIN tier."
        ),
        inputs=("request",),
        outputs=("status",),
        dependencies=("app.routers.role_ui",),
