...
Migrated from app/routers/brain.py into the brain SDK module.
All imports remain absolute since brain is a RESEARCH module that depends
...
async def emit_event(request: EventRequest, brain: PositronicBrain = Depends(get_brain)):
    ...
    event = BrainEvent(event_type=event_type, source_module=source_module, data=request.data)
...
async def trigger_workflow(request: WorkflowRequest, brain: PositronicBrain = Depends(get_brain)):
    ...
    """
    workflow_id = await brain.trigger_workflow(request.workflow_name, request.data)

    return {"workflow_id": workflow_id, "name": request.workflow_name, "status": "started"}
    ...
    try:
        while True:
            ...
            if msg_type == "emit_event":
                ...
                await brain.update_state(data["key"], data["value"], ModuleType.UI)

            elif msg_type == "trigger_workflow":
                # Client wants to trigger workflow
                workflow_id = await brain.trigger_workflow(data["workflow_name"], data.get("data", {}))
                await websocket.send_json({"type": "workflow_started", "workflow_id": workflow_id})

            elif msg_type == "get_state":
                # Client requests current state
                await websocket.send_json({"type": "state", "state": brain.get_state()})
