...
class AutoModeOrchestrator:
    ...
    async def run_full_auto_analysis(
        ...
        try:
            # 1. Build timeline
            try:
                timeline_result = await self.timeline_builder.build_from_text(document_content, document_id=doc_id)
                results["timeline_events"] = (
                    [event.to_dict() for event in timeline_result.events] if hasattr(timeline_result, "events") else []
                )
            ...
            try:
                calendar_events = await self.calendar_service.generate_events_from_timeline(results["timeline_events"])
            ...
            try:
                case_data = {"documents": {doc_id: {"full_text": document_content, **document_metadata}}, "issues": {}}
            ...
            try:
                ...
                results["complaints"] = [
                    {"agency": getattr(agency, "name", str(agency)), "type": str(getattr(agency, "type", "unknown"))}
                    for agency in (recommended_agencies or [])
                ]
            except Exception as e:
            ...
            try:
                tactics = self.tactics_engine.run_decision_tree(timeline_events=results["timeline_events"])
                if asyncio.iscoroutine(tactics):
                    tactics = await tactics
                results["proactive_tactics"] = [
                    tactic.to_dict() if hasattr(tactic, "to_dict") else tactic.__dict__ for tactic in (tactics or [])
                ]
            except Exception as e:
            ...
            try:
                summary_result = await self.summary_service.generate_summary(doc_id, filename, results)
            ...
            try:
                await event_bus.publish(
                    ...
                    {"doc_id": doc_id, "user_id": user_id, "results": results, "summary": results.get("summary")},
                )
            except Exception as e:
