...
try:
    ...
    from app.core.security import StorageUser, require_user, yellow_access  # noqa: F401
...
async def get_packet_checklist(request: Request) -> dict[str, Any]:
    ...
    # Get processed documents
    processed_docs = _get_processed_documents(user_id)
    processed_doc_types = {d.get("doc_type") for d in processed_docs if d.get("doc_type")}
...
def generate_evidence_index(docs: dict, extractions: dict, highlights: dict) -> str:
    ...
    for i, (_doc_id, doc) in enumerate(docs.items(), 1):
    ...
    if extractions:
        ...
        for i, (_ext_id, ext) in enumerate(extractions.items(), 1):
...
def generate_highlights_summary(highlights: dict) -> str:
    ...
    for _h_id, h in highlights.items():
