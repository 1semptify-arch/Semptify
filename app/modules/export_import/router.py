...
class ExportRequestSchema(BaseModel):
    ...
    export_type: str = Field(
        ..., description="Export type: all_data, documents_only, timeline_only, contacts_only, user_profile, audit_log"
    )
...
async def create_export_request_endpoint(request: ExportRequestSchema, user: StorageUser = Depends(red_access)):
    """
    ...
    try:
        ...
        export_id = create_export_request(
            user_id=user.user_id, export_type=export_type, format=format_type, filters=request.filters or {}
        )
...
async def process_export_endpoint(export_id: str, user: StorageUser = Depends(red_access)):
    """
    ...
    try:
        ...
        return {
            ...
            else "Export processing started",
...
async def get_export_status_endpoint(export_id: str, user: StorageUser = Depends(red_access)):
    """
...
async def download_export_endpoint(export_id: str, user: StorageUser = Depends(red_access)):
    """
...
async def delete_export_endpoint(export_id: str, user: StorageUser = Depends(red_access)):
    """
    ...
    try:
        ...
        return {"success": True, "export_id": export_id, "message": "Export deleted successfully"}
...
async def prepare_import_endpoint(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid import format: {format}")

        # Create import request
        from app.core.data_export_import import create_import_request

        import_id = create_import_request(
            user_id=user.user_id, import_type=import_type, format=format_type, validation_required=validation_required
        )
...
async def upload_import_file_endpoint(
    import_id: str, file: UploadFile = File(..., description="Import file"), user: StorageUser = Depends(red_access)
):
    """
...
async def get_export_statistics_endpoint(user: StorageUser = Depends(red_access)):
    """
    ...
    try:
        ...
        user_stats = {
            ...
            "completed_exports": len([exp for exp in user_exports if exp["status"] == "completed"]),
            "failed_exports": len([exp for exp in user_exports if exp["status"] == "failed"]),
            "pending_exports": len([exp for exp in user_exports if exp["status"] == "pending"]),
        }
...
async def get_supported_formats_endpoint():
    ...
    return {
        ...
        "import_formats": [
            ...
            {"format": "csv", "name": "CSV", "description": "Comma-separated values", "supported_types": ["contacts"]},
            {"format": "zip", "name": "ZIP", "description": "Compressed archive", "supported_types": ["documents"]},
        ],
        "merge_strategies": [
            {"strategy": "skip", "name": "Skip Existing", "description": "Skip items that already exist"},
            {"strategy": "overwrite", "name": "Overwrite", "description": "Replace existing items"},
            {"strategy": "merge", "name": "Merge", "description": "Merge with existing items"},
        ],
        "limits": {
            "max_file_size": "100MB",
...
@router.post("/cleanup")
async def cleanup_exports_endpoint(user: StorageUser = Depends(red_access)):
    """
