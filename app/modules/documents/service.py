...
class EventExtractor:
    ...
    EVENT_CONTEXTS = [
        ...
        (
            r"(?:repair\w*|maintenanc\w*)\s*(?:request\w*|schedul\w*)?\s*(?:on|for)?\s*",
            "Repair/Maintenance",
            "maintenance",
            0.8,
        ),
    ...
    def __init__(self):
        ...
        self._context_patterns = [
            (re.compile(p, re.IGNORECASE), title, etype, conf) for p, title, etype, conf in self.EVENT_CONTEXTS
        ]
    ...
    def extract_events(self, text: str, doc_type: str = "unknown") -> list[ExtractedEvent]:
        ...
        for chunk in chunks:
            ...
            for date, date_str, position in dates_found:
                ...
                event_type, title, confidence = self._classify_event(context_before, context_after, doc_type)
    ...
    def _should_exclude(self, context: str) -> bool:
        ...
        return any(pattern.search(context) for pattern in self._exclude_patterns)
    ...
    def _classify_event(self, context_before: str, context_after: str, doc_type: str) -> tuple[str, str, float]:
        ...
        for pattern, title, event_type, confidence in self._context_patterns:
            if pattern.search(full_context) and confidence > best_confidence:
