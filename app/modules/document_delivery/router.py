...
@router.get("/inbox/page", response_class=HTMLResponse)
...
async def send_document(
    ...
    message: str | None = Form(None, description="Optional message to recipient"),
    ...
    if sender_role not in SENDER_ROLES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Role '{sender_role}' cannot send documents")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid delivery_type: {delivery_type}")
...
async def get_outbox(
    ...
    sender_role = _get_user_role(user)
    if sender_role not in SENDER_ROLES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only professional roles can view outbox")
