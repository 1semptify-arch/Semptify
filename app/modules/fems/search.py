...
async def search_evidence(keywords: str, db: AsyncSession) -> list[dict]:
    ...
    docs = (await db.execute(
        select(FemsDocument).where(FemsDocument.extracted_text.ilike(kw))
    )).scalars().all()
    ...
    chunks = (await db.execute(
        select(FemsChunk).where(FemsChunk.content.ilike(kw))
    )).scalars().all()
...
async def search_by_phone(number: str, db: AsyncSession) -> list[dict]:
    ...
    phone = (await db.execute(
        select(FemsPhoneNumber).where(FemsPhoneNumber.number.ilike(f"%{number}%"))
    )).scalars().all()

    results = []
    for p in phone:
        links = (await db.execute(
            select(FemsDocumentPhone).where(FemsDocumentPhone.phone_id == p.id)
        )).scalars().all()

        for link in links:
            doc = (await db.execute(
                select(FemsDocument).where(FemsDocument.id == link.document_id)
            )).scalar_one_or_none()
