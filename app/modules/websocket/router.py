...
"""
# Migrated from app/routers/websocket.py into the websocket SDK module.
        ...
        while True:
            try:
                ...
                if message.get("type") == "ping":
                    ...
                    await websocket.send_json({"type": "jobs", "jobs": user_jobs})
                    ...
                    await websocket.send_json({"type": "job_status", "job_id": job_id, "status": job_status})
...
async def get_user_connections(
    ...
    return {"user_id": user_id, "connections": connections, "connection_count": len(connections)}
...
async def send_notification_to_user(
    ...
    message = WebSocketMessage(type=message_type, data=message_data, timestamp=utc_now(), user_id=user_id)
...
async def broadcast_notification(
    ...
    message = WebSocketMessage(type=message_type, data=message_data, timestamp=utc_now())

    success = await ws_manager.broadcast_to_all(message)

    return {"success": success, "message": "Broadcast sent" if success else "Failed to broadcast"}
