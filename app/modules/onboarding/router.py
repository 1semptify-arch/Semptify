...
ONBOARDING SOLO — tenant-only onboarding.
...
from app.core.vault_paths import SYSTEM_FOLDER
...
def create_router(config: OnboardingConfig) -> APIRouter:
    ...
    async def role_select_static(request: Request, fresh: str | None = Query(None)):
        """Redirect legacy /onboarding/role-select to the flow entry.

        ONBOARDING SOLO: there is no role selection — this just forwards to
        wherever the flow's entry stage currently points (provider selection).
        Preserves the ``?fresh=true`` cookie-clear behavior on the redirect.
        """
        role_stage = navigation.get_stage("role_select")
        role_path = role_stage.path if role_stage else "/onboarding/providers"
        response = ssot_redirect(role_path, context="role-select legacy duplicate")
        if fresh == "true":
    ...
    async def role_selection_page():
        """Legacy role-selection URL — onboarding solo removed the page;
        redirect stale links/bookmarks to provider selection."""
        return ssot_redirect("/onboarding/providers", context="select-role removed (onboarding solo)")

    @router.get("/start")
    async def onboarding_start(
        semptify_uid: str | None = Cookie(None),
    ):
        """Smart entry: returning user → reconnect, new user → onboarding flow."""
        if semptify_uid:
    ...
    async def onboarding_oauth_start(
        ...
        ONBOARDING SOLO: every onboarding account is a tenant — there is no
        role parameter to accept, ignore, or tamper with.
        """
        role = "tenant"
        logger.info("=== OAUTH REQUEST RECEIVED ===")
        logger.info("onboarding_oauth_start: provider=%s force_fresh=%s", provider, force_fresh)
...
{tb}
    ...
    async def vault_security(
        ...
        """Step 2: Write encrypted token backup, live probe, then mark vault_initialized."""
        import asyncio
        ...
        try:
            ...
            # Live write/read-back probe under the Semptify-owned .semptify
            # system folder. It writes, reads, compares, and deletes a file this
            # call itself creates. It never touches the user's document and it
            # does not depend on the background file-creation task above.
            try:
                ...
                probe_path = f"{SYSTEM_FOLDER}/{probe_name}"
                ...
                read_back = await asyncio.wait_for(
                    storage.download_file(probe_path),
                    timeout=10.0,
                )
                if read_back != probe_bytes:
                    raise ValueError("Read-back mismatch — vault storage is unreliable")
                await asyncio.wait_for(storage.delete_file(probe_path), timeout=5.0)
            except TimeoutError:
    ...
    async def vault_verify(
        ...
        Routes the uploaded document through VaultUploadService (the canonical
        full pipeline): certificate → registry → overlay → timeline extraction
        → event bus → positronic mesh workflows.

        Marks the document_uploaded completion gate only after the pipeline
        succeeds. The live write/read-back probe has already passed in step 2
        (/api/vault/security); it is not repeated here.

        A document is REQUIRED — posted in this request, or already sitting in
        the vault from a prior partial attempt (detect-first heal). There is
        no held/pending document path.
        """
        import asyncio
...
function selectProvider(provider) {{
    window.location.href = '{config.route_prefix}/auth/' + provider;
}}
</script>
