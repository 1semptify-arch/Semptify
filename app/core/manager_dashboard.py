...
def get_dashboard_stats(organization_id: str, db_session) -> dict[str, Any]:
    """
    ...
    org_users = db_session.query(User).filter(User.id.like(f"{organization_id}%")).all()
    ...
    new_cases = db_session.query(User).filter(User.id.like(f"{organization_id}%"), User.created_at >= week_ago).count()
...
def get_recent_cases(organization_id: str, db_session, limit: int = 10) -> list[dict[str, Any]]:
    """
    ...
    users = (
        ...
        .filter(User.id.like(f"{organization_id}%"))
    ...
    for user in users:
        ...
        doc_count = db_session.query(Document).filter(Document.user_id == user.id).count()
...
def get_staff_list(organization_id: str, db_session) -> list[dict[str, Any]]:
    """
    ...
    redeemed_codes = (
        ...
        .filter(InviteCode.organization_id == organization_id, InviteCode.used_by.isnot(None))
        .all()
    )
...
def get_pending_signatures(organization_id: str, db_session) -> list[dict[str, Any]]:
    """
...
def get_recent_activity(organization_id: str, db_session, limit: int = 20) -> list[dict[str, Any]]:
    """
...
def get_organization_info(manager_user_id: str, db_session) -> dict[str, Any]:
    """
