...
class JobQueue:
    ...
    def __init__(self, max_size: int = 10000):
        ...
        self._queues: dict[JobPriority, deque] = {priority: deque() for priority in JobPriority}
...
class JobProcessor:
    ...
    def submit_job(
        ...
        **kwargs,
        ...
        job = Job(id=job_id, type=job_type, payload=payload, priority=priority, user_id=user_id, **kwargs)
    ...
    def _process_job(self, job: Job):
        ...
        try:
            ...
            result = asyncio.run(self._execute_with_timeout(handler, job.payload, job.timeout_seconds))
    ...
    def _send_job_notification(self, job: Job, status: str):
        ...
        try:
            ...
            if job.user_id:
                asyncio.run(
                    ws_manager.send_job_status_update(
                        user_id=job.user_id, job_id=job.id, status=status, progress=job.progress, result=job.result
                    )
                )

            # Also broadcast to job monitoring subscription
            asyncio.run(
                ws_manager.broadcast_to_subscription(
                    "job_monitoring", WebSocketMessage(type="job_update", data=notification_data, timestamp=utc_now())
                )
            )
    ...
    async def _execute_with_timeout(self, handler: Callable, payload: dict[str, Any], timeout_seconds: int) -> Any:
    ...
    def _cleanup_completed_jobs(self):
        ...
        if len(self.completed_jobs) > self.max_completed_jobs:
            # Remove oldest jobs
            sorted_jobs = sorted(self.completed_jobs.items(), key=lambda x: x[1].completed_at or x[1].created_at)
    ...
    def get_user_jobs(self, user_id: str, status: JobStatus | None = None) -> list[dict[str, Any]]:
    ...
    def get_queue_stats(self) -> dict[str, Any]:
        ...
        queue_sizes = {priority.name.lower(): len(queue) for priority, queue in self.job_queue._queues.items()}
...
def register_default_handlers(processor: JobProcessor):
    ...
    async def document_analysis_handler(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "results": {"pages": 10, "word_count": 5000, "language": "en", "confidence": 0.95},
        }
    ...
    async def thumbnail_generation_handler(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "document_id": document_id,
            "thumbnails": [
                {"page": page, "url": f"/thumbnails/{document_id}/page_{page}.jpg", "size": "200x300"}
        ...
        try:
            ...
            if not raw_text:
                ...
                try:
                    ...
                    provider_code = get_provider_from_user_id(user_id) or getattr(doc, "storage_provider", None)
                    if not provider_code or provider_code == "local":
                        ...
                        async with get_db_session() as db:
                            is_valid, token, _ = await ensure_valid_token(user_id, db)
                        ...
                        if overlay_resp.success:
                            ...
                            raise RuntimeError(f"Overlay creation failed: {overlay_resp.message}")
...
# Helper functions
def submit_document_analysis_job(document_id: str, analysis_type: str = "basic", user_id: str = None) -> str:
...
def submit_thumbnail_generation_job(document_id: str, page_numbers: list[int] = None, user_id: str = None) -> str:
    """Submit thumbnail generation job."""
    processor = get_job_processor()
    return processor.submit_job(
        job_type="thumbnail_generation",
        payload={"document_id": document_id, "page_numbers": page_numbers or [1]},
...
def get_user_jobs(user_id: str, status: str | None = None) -> list[dict[str, Any]]:
    """Get user's jobs."""
    processor = get_job_processor()
