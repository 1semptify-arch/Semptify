...
def create_error_response(error: SemptifyError, include_details: bool = False) -> JSONResponse:
...
async def semptify_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    if "text/html" in accept and "application/json" not in accept:
        ...
        return ssot_redirect(help_url, context="error handler down")
    ...
    if isinstance(exc, SemptifyError):
        ...
        user_error = SemptifyError(
            message=str(exc.detail), error_code="HTTP_ERROR", status_code=exc.status_code, user_message=str(exc.detail)
        )
        ...
        user_error = SemptifyError(
            message=exc.detail, error_code="HTTP_ERROR", status_code=exc.status_code, user_message=exc.detail
        )
        return create_error_response(user_error)
...
def handle_component_error(component_name: str, error: Exception, user_message: str | None = None) -> SemptifyError:
...
class RateLimitError(SemptifyError):
    ...
    def __init__(
        self, message: str = "Too many requests", retry_after: int | None = None, user_message: str | None = None
    ):
        details = {}
        if retry_after:
        ...
        super().__init__(
            ...
            or f"Too many requests. Please wait {retry_after or 'a few'} seconds and try again.",
        )
...
def handle_file_upload_error(error: Exception, filename: str) -> SemptifyError:
    ...
    return StorageError(
        message=f"Upload error for {filename}: {str(error)}", error_code="UPLOAD_ERROR", details={"filename": filename}
    )
...
def handle_oauth_error(error: Exception, provider: str) -> SemptifyError:
    ...
    return AuthenticationError(
        message=f"OAuth error for {provider}: {str(error)}", error_code="OAUTH_ERROR", details={"provider": provider}
    )
