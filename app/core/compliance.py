...
def _describe_component(path: Path, category: str) -> ModuleCompliance:
    ...
    if category == "router":
        privacy_scope = f"Handles HTTP routing for {display}. Must enforce auth, validate input, and avoid exposing unauthorized PII."
        evidence_role = f"Serves API endpoints for {item}, which support the application workflows and evidence flows."
        security_notes = "Require authorization, request validation, and logging for all exposed routes."
        next_action = f"Verify router {display} enforces access control and logs sensitive actions."
    else:
        privacy_scope = f"Implements service logic for {display}. Must isolate sensitive data, limit retention, and avoid unnecessary PII processing."
        evidence_role = (
        ...
        next_action = f"Review service {display} for data handling, audit logging, and access restrictions."
...
MODULE_COMPLIANCE_INVENTORY: list[ModuleCompliance] = (
    ...
    + _discover_components(ROUTERS_DIR, "router")
    + _discover_components(SERVICES_DIR, "service")
)
...
def validate_app_compliance(settings: Settings) -> None:
    ...
    missing_files = [
        module.file_path for module in MODULE_COMPLIANCE_INVENTORY if not (ROOT_DIR / module.file_path).exists()
    ]
    if missing_files:
        logger.warning(
