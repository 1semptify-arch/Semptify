...
class DataExportImportManager:
    ...
    def create_export_request(
        ...
        export_id = f"export_{utc_now().strftime('%Y%m%d_%H%M%S')}_{hashlib.md5(user_id.encode()).hexdigest()[:8]}"  # noqa: S324 - non-security unique id
    ...
    def create_import_request(
        ...
        import_id = f"import_{utc_now().strftime('%Y%m%d_%H%M%S')}_{hashlib.md5(user_id.encode()).hexdigest()[:8]}"  # noqa: S324 - non-security unique id
