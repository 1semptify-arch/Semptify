...
class PerformanceMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        if psutil is None:
            # psutil not installed — record sentinel values instead of crashing.
            self._store_metric("system.cpu_percent", -1.0, "percent")
            self._store_metric("system.memory_percent", -1.0, "percent")
            self._store_metric("system.disk_usage_percent", -1.0, "percent")
            self._store_metric("system.active_connections", -1, "count")
            return
    ...
    def _store_metric(self, name: str, value: float, unit: str, tags: dict[str, str] = None):
        ...
        metric = PerformanceMetric(name=name, value=value, unit=unit, timestamp=utc_now(), tags=tags)
    ...
    def record_request(
        ...
        self._store_metric(
            f"request.duration.{endpoint}", duration_ms, "ms", {"method": method, "status_code": str(status_code)}
        )
        ...
        if duration_ms > self.slow_request_threshold:
            ...
            self._store_metric(
                "slow_requests.count", 1, "count", {"endpoint": endpoint, "method": method, "duration_ms": duration_ms}
            )
        ...
        if is_error:
            self._store_metric(
                "request.errors", 1, "count", {"endpoint": endpoint, "method": method, "status_code": str(status_code)}
            )

    def record_database_query(self, query: str, duration_ms: float, rows_affected: int = None):
        """Record database query performance."""
        self._store_metric("database.query_duration", duration_ms, "ms", {"query_type": self._classify_query(query)})
    ...
    def record_storage_operation(self, operation: str, duration_ms: float, file_size: int = None):
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        # Request metrics
        recent_requests = [req for req in self.request_metrics if req.timestamp >= one_hour_ago]
        ...
        # Slow requests
        slow_requests_count = len([req for req in recent_requests if req.duration_ms > self.slow_request_threshold])
        ...
        return {
            ...
            "slow_queries": {
                ...
                "database_queries": len(
                    [sq for sq in self.slow_queries if isinstance(sq, dict) and sq.get("type") == "database"]
                ),
    ...
    def get_endpoint_performance(self, endpoint: str) -> dict[str, Any]:
        ...
        endpoint_requests = [req for req in self.request_metrics if req.endpoint == endpoint]
    ...
    def get_metrics_history(self, metric_name: str, hours: int = 24) -> list[dict[str, Any]]:
        ...
        historical_metrics = [metric for metric in self.metrics[metric_name] if metric.timestamp >= cutoff_time]
    ...
    def export_performance_data(self, hours: int = 24) -> dict[str, Any]:
        ...
        # Filter recent data
        recent_requests = [req.to_dict() for req in self.request_metrics if req.timestamp >= cutoff_time]

        recent_system = [sys.to_dict() for sys in self.system_metrics if sys.timestamp >= cutoff_time]
...
def record_request_performance(
    endpoint: str, method: str, status_code: int, duration_ms: float, user_id: str = None, ip_address: str = None
):
    """Record HTTP request performance."""
