...
class PostgresFTSService:
    ...
    async def search_documents(
        self, query: str, user_id: str | None = None, limit: int = 20, highlight_max_length: int = 150
    ) -> list[FTSResult]:
        """
        ...
        return [
            FTSResult(document_id=row.id, rank=row.rank, headline=row.headline or row.title, table_name="documents")
            for row in result.fetchall()
        ]

    async def search_vault_items(self, query: str, user_id: str | None = None, limit: int = 20) -> list[FTSResult]:
        """
        ...
        return [
            FTSResult(document_id=row.id, rank=row.rank, headline=row.headline or row.title, table_name="vault_items")
            for row in result.fetchall()
        ]

    async def hybrid_search(self, query: str, user_id: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
        """
    ...
    async def get_search_suggestions(self, partial: str, user_id: str | None = None, limit: int = 10) -> list[str]:
        """
