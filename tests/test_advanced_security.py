...
def test_generate_qr_code_returns_base64_png():
    """generate_qr_code returns a base64 PNG when qrcode is available."""
    manager = TwoFactorAuthManager()
    qr_b64 = manager.generate_qr_code("secret", "user@example.com")
    assert isinstance(qr_b64, str)
    assert qr_b64.startswith("iVBORw0KGgo")  # PNG base64 header
...
def test_revoke_all_user_sessions():
    ...
    manager = SessionManager()
    _ = manager.create_session("user_4", "127.0.0.1", "pytest")
    sid2 = manager.create_session("user_4", "127.0.0.2", "pytest")
    _ = manager.create_session("user_4", "127.0.0.3", "pytest")
