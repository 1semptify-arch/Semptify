...
async def cleanup_test_db():
    """Clean up legacy test database files after tests."""
    yield
    for db_name in ("test_semptify.db", "test_semptify.db-shm", "test_semptify.db-wal"):
        path = Path(db_name)
        if path.exists():
...
def mock_azure_ai():
    ...
    with patch("app.services.azure_ai.AzureAIService") as mock:
        ...
        instance.analyze_document = AsyncMock(
            return_value={
                "content": "Mock document content",
                "pages": [{"page_number": 1, "text": "Mock text"}],
            }
        )
        instance.classify_document = AsyncMock(
            return_value={
                "doc_type": "lease",
                "confidence": 0.95,
            }
...
def mock_openai():
    ...
    with patch("httpx.AsyncClient") as mock:
        ...
        mock_response.json.return_value = {
            "choices": [{"message": {"content": "This is a mock AI response for tenant rights."}}]
        }
