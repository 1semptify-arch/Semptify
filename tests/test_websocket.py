...
class MockWebSocket:
    ...
    async def send_text(self, data):
        # Parse JSON text so message list remains uniformly inspectable.
        try:
...
class TestWebSocketEvents:
    ...
    async def test_websocket_events_connection(self):
        ...
        # Check welcome message was sent (manager sends connection_established first)
        assert len(ws.messages_sent) >= 2
        welcome = ws.messages_sent[1]
        assert welcome["type"] == "connected"
        assert welcome["user_id"] == "test_user_123"
        assert "Connected to Semptify" in welcome["message"]
...
class TestBrainWebSocket:
    ...
    @pytest.mark.skip(reason="Positronic Brain REST router is disabled in product manifest")
    ...
    async def test_brain_websocket_trigger_workflow(self):
        ...
        ws = MockWebSocket()
        ws.queue_message({"type": "trigger_workflow", "workflow_name": "test_workflow", "data": {"test": True}})
...
class TestEventBusWebSocketIntegration:
    ...
    async def test_event_bus_publish_event(self):
        ...
        await event_bus.publish(EventType.DOCUMENT_ADDED, {"document_id": "test_doc"}, user_id="test_user")
...
class TestWebSocketPerformance:
    ...
    async def test_message_order_preserved(self):
        ...
        # Check order: connection_established, connected, pong, subscribed, pong
        types = [m.get("type") for m in ws.messages_sent]
        assert types[1] == "connected"
...
def patch_websocket_auth():
    """Bypass user-storage validation in the websocket router so message-protocol
    tests can drive the endpoint directly without real OAuth sessions.
    """
    import importlib

    websocket_router = importlib.import_module("app.modules.websocket.router")

    original_is_valid = websocket_router.is_valid_user_storage
    websocket_router.is_valid_user_storage = lambda user_id: True
    yield
    websocket_router.is_valid_user_storage = original_is_valid


@pytest.fixture
def test_client():
    """Create an authenticated test client so HTTP endpoints under /ws work."""
    test_uid = "GU7x9kM2pQ"
    signed_uid = sign_user_id(test_uid)
    ...
    client = TestClient(app, cookies={"semptify_uid": signed_uid})
    yield client
