...
**Skip this step by default.** Run it only if Brad says he has been using the admin dashboard via ops elevation — then use whatever Render log/MCP tooling is available to search Render logs for `FIXIT_REPORT` in the last 7 days, parse the `FIXIT_REPORT|...` lines, and tell the user what was found.
