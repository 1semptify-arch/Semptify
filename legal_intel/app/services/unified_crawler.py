...
from ..crawlers import courtlistener, mcro, plainsite, sos
...
async def upsert_attorney(db: AsyncSession, bar_number: str, state: str = "MN") -> Attorney:
    ...
    if not attorney:
        attorney = Attorney(name="", bar_number=bar_number, state=state, last_seen=datetime.now(UTC))
        db.add(attorney)
        await db.commit()
        await db.refresh(attorney)
    else:
        attorney.last_seen = datetime.now(UTC)
        await db.commit()
    return attorney


async def upsert_entity(db: AsyncSession, entity_data: dict) -> Entity | None:
    """
    ...
    result = await db.execute(Entity.__table__.select().where(Entity.sos_id == entity_data.get("sos_id")))
...
async def upsert_case(db: AsyncSession, attorney: Attorney, case_data: dict) -> Case:
    ...
    if not case:
        ...
        case.last_crawled = datetime.now(UTC)
...
def extract_entity_from_case_title(case_title: str) -> str | None:
    """
