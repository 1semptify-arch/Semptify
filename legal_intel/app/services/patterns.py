...
async def compute_entity_patterns(db: AsyncSession, entity_id: int) -> dict:
    ...
    """
    q = await db.execute(select(Case).options(selectinload(Case.attorney)).where(Case.entity_id == entity_id))
    cases = q.scalars().all()
...
async def detect_shell_llc_clusters(db: AsyncSession) -> list[dict]:
    ...
    significant_agent_clusters = [
        {"agent": agent, "entities": entities} for agent, entities in agent_clusters.items() if len(entities) > 1
    ]
