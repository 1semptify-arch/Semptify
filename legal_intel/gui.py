...
async def show_attorney_patterns(bar_number, log_box):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code == 200:
                patterns = resp.json()
                log("✓ Patterns retrieved:", log_box)
...
async def show_shell_llc_clusters(log_box):
    ...
    try:
        log("Fetching shell LLC clusters", log_box)
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code == 200:
                clusters = resp.json()
                log("✓ Clusters retrieved:", log_box)
...
def build_gui():
    ...
    # Title
    title_label = tk.Label(root, text="Legal Intelligence Engine", font=("Arial", 16, "bold"), bg="#f0f0f0")
    ...
    # Clear log button
    clear_btn = tk.Button(
        log_frame, text="Clear Log", command=lambda: clear_log(log_box), bg="#f44336", fg="white", width=15
    )
