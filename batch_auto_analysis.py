    ...
    async def analyze_document(self, doc_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not doc_content or len(doc_content.strip()) < 100:
                ...
                return {"status": "skipped", "reason": "insufficient_content", **doc_info}
            ...
            results = await orchestrator.run_full_auto_analysis(
                ...
                document_metadata={"uploaded_by": doc_info["user_id"], "file_size": doc_info["size"]},
            )
...
async def main():
    ...
    # Run batch analysis
    await analyzer.run_batch_analysis(limit=5)  # Analyze first 5 docs
    ...
    for i, action in enumerate(report["all_urgent_actions"][:3], 1):
        print(
            f"  {i}. [{action.get('severity', 'N/A').upper()}] {action.get('message', 'N/A')} ({action.get('deadline', 'N/A')})"
        )
