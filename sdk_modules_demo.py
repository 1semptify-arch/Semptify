...
# Complaints
complaints_def = ModuleDefinition(name="complaints", display_name="Complaints", description="Complaint filing")
complaints_sdk = ModuleSDK(complaints_def)


@complaints_sdk.action(
    "file_complaint",
    required_params=["target_agency", "violation_type", "facts", "language"],
    produces=["complaint_record"],
)
async def file_complaint(user_id, params, context):
    record = {"id": f"cmp_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}", **params, "status": "submitted"}
...
async def update_status(user_id, params, context):
    return {"complaint_status": {**params, "updated_at": datetime.now(UTC).isoformat()}}
...
async def export_zip(user_id, params, context):
    return {
        "zip_bundle": {"complaint_id": params["complaint_id"], "zip_path": f"/exports/{params['complaint_id']}.zip"}
    }
...
# Fraud Exposure
fraud_def = ModuleDefinition(name="fraud_exposure", display_name="Fraud Exposure", description="Fraud analysis")
fraud_sdk = ModuleSDK(fraud_def)


@fraud_sdk.action(
    "analyze_fraud", required_params=["landlord_id", "case_docs", "subsidies", "lenders"], produces=["fraud_report"]
)
async def analyze_fraud(user_id, params, context):
    ...
    report = {"landlord_id": params["landlord_id"], "findings": findings, "created_at": datetime.now(UTC).isoformat()}
...
# Public Exposure
public_def = ModuleDefinition(name="public_exposure", display_name="Public Exposure", description="Press releases")
public_sdk = ModuleSDK(public_def)


@public_sdk.action(
    "generate_press_release",
    required_params=["property", "violations", "contact", "bundle_link", "language"],
    produces=["press_release"],
)
async def generate_press_release(user_id, params, context):
    return {
        "press_release": {
            "headline": "Tenants expose misconduct",
            "lede": f"{params['property']} issues: {params['violations']}",
            "cta": f"Contact {params['contact']}",
            "bundle": params["bundle_link"],
        }
    }
...
async def launch_campaign(payload: dict[str, Any] = Body(...)):
    ...
    press = await public_sdk.invoke_action(
        "public_exposure", "generate_press_release", "user", payload.get("press", {})
    )
    export = await complaints_sdk.invoke_action(
        "complaints", "export_zip", "user", {"complaint_id": complaint["complaint_record"]["id"]}
    )
