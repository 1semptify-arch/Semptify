...
async def main() -> int:
    ...
    is_postgres = (
        "postgresql+asyncpg" in db_url or db_url.startswith("postgresql://") or db_url.startswith("postgres://")
    )
    ...
    async with get_db_session() as db:
        row = (
            await db.execute(select(DocumentPipelineIndex).where(DocumentPipelineIndex.doc_id == doc_id))
        ).scalar_one_or_none()
