...
class MNCourtCrawler:
        ...
        for url, court_type in urls:
            ...
            try:
                ...
                for link in pdf_links[:25]:
                    ...
                    downloaded = await self.download_document(
                        full_url, OPINIONS_DIR, doc_type="opinion", court=court_type, year=datetime.now().year
                    )
                    if downloaded:
                        count += 1
    ...
    def save_metadata(self):
        ...
        existing = json.loads(index_file.read_text(encoding="utf-8")) if index_file.exists() else {"documents": []}
