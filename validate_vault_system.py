    ...
    def validate_vault_component_structure(self):
        ...
        try:
            ...
            if not vault_component_path.exists():
                self.log_validation("Vault Component Structure", False, "Vault component file not found")
                return False

            # Read component content
            with open(vault_component_path, encoding="utf-8") as f:
                content = f.read()
            ...
            if missing_elements:
                self.log_validation(
                    "Vault Component Structure", False, f"Missing required elements: {', '.join(missing_elements)}"
                )
            else:
                self.log_validation("Vault Component Structure", True, "All required elements present")

            # Check for CSS syntax issues
            css_issues = self._check_css_syntax(content)
            if css_issues:
                self.log_validation("Vault Component CSS", False, f"CSS syntax issues found: {', '.join(css_issues)}")
            else:
                self.log_validation("Vault Component CSS", True, "CSS syntax is valid")
            ...
            if missing_functions:
                self.log_validation(
                    "Vault Component JavaScript", False, f"Missing JavaScript functions: {', '.join(missing_functions)}"
                )
            else:
                self.log_validation("Vault Component JavaScript", True, "All required JavaScript functions present")

            return len(missing_elements) == 0 and len(missing_functions) == 0 and len(css_issues) == 0

        except Exception as e:
            self.log_validation("Vault Component Structure", False, f"Validation failed: {str(e)}")
            return False
    ...
    def validate_backend_integration(self):
        ...
        try:
            ...
            if not vault_router_path.exists():
                self.log_validation("Vault Backend Integration", False, "Vault router file not found")
                return False

            # Read router content
            with open(vault_router_path, encoding="utf-8") as f:
                content = f.read()
            ...
            if missing_endpoints:
                self.log_validation(
                    "Vault Backend Integration", False, f"Missing required endpoints: {', '.join(missing_endpoints)}"
                )
            else:
                self.log_validation("Vault Backend Integration", True, "All required endpoints present")
            ...
            if missing_imports:
                self.log_validation(
                    "Vault Backend Imports", False, f"Missing required imports: {', '.join(missing_imports)}"
                )
            else:
                self.log_validation("Vault Backend Imports", True, "All required imports present")

            return len(missing_endpoints) == 0 and len(missing_imports) == 0

        except Exception as e:
            self.log_validation("Vault Backend Integration", False, f"Validation failed: {str(e)}")
            return False
    ...
    def validate_page_integration(self):
        ...
        try:
            ...
            if not tenant_dashboard_path.exists():
                self.log_validation("Page Integration", False, "Tenant dashboard not found")
                return False
            ...
            if "vault-sidebar-clean.html" not in content:
                self.log_validation("Page Integration", False, "Vault component not included in tenant dashboard")
            else:
                self.log_validation("Page Integration", True, "Vault component included in tenant dashboard")

            # Check for responsive layout adjustments
            required_css = ["margin-right: 320px", ".vault-sidebar {", ".dashboard-content {"]
            ...
            if missing_css:
                self.log_validation(
                    "Page Responsive Layout", False, f"Missing responsive CSS: {', '.join(missing_css)}"
                )
            else:
                self.log_validation("Page Responsive Layout", True, "Responsive layout adjustments present")

            return "vault-sidebar-clean.html" in content and len(missing_css) == 0

        except Exception as e:
            self.log_validation("Page Integration", False, f"Validation failed: {str(e)}")
            return False
    ...
    def validate_main_app_integration(self):
        ...
        try:
            ...
            if not main_app_path.exists():
                self.log_validation("Main App Integration", False, "Main app file not found")
                return False
            ...
            if "from app.routers import vault" not in content:
                self.log_validation("Main App Integration", False, "Vault router not imported in main app")
            else:
                self.log_validation("Main App Integration", True, "Vault router imported in main app")

            # Check for vault router inclusion
            if "fastapi_app.include_router(vault.router" not in content:
                self.log_validation("Main App Integration", False, "Vault router not included in main app")
            else:
                self.log_validation("Main App Integration", True, "Vault router included in main app")

            return "from app.routers import vault" in content and "fastapi_app.include_router(vault.router" in content

        except Exception as e:
            self.log_validation("Main App Integration", False, f"Validation failed: {str(e)}")
            return False
    ...
    def validate_oauth_storage_integration(self):
        ...
        try:
            ...
            if not storage_router_path.exists():
                self.log_validation("OAuth Storage Integration", False, "Storage router not found")
                return False
            ...
            if not vault_service_path.exists():
                self.log_validation("OAuth Storage Integration", False, "Vault service not found")
            else:
                self.log_validation("OAuth Storage Integration", True, "Vault service found")

            # Read storage router content
            with open(storage_router_path, encoding="utf-8") as f:
                content = f.read()
            ...
            if len(found_providers) == 0:
                self.log_validation("OAuth Storage Integration", False, "No OAuth providers found in storage router")
            else:
                self.log_validation(
                    "OAuth Storage Integration", True, f"OAuth providers found: {', '.join(found_providers)}"
                )

            return len(found_providers) > 0 and vault_service_path.exists()

        except Exception as e:
            self.log_validation("OAuth Storage Integration", False, f"Validation failed: {str(e)}")
            return False
    ...
    def validate_credentials_workbook(self):
        ...
        try:
            ...
            if not workbook_path.exists():
                self.log_validation("Credentials Workbook", False, "Revealed credentials workbook not found")
                return False
            ...
            if len(found_fields) == 0:
                self.log_validation("Credentials Workbook", False, "No credential fields found in workbook")
            else:
                self.log_validation("Credentials Workbook", True, f"Found {len(found_fields)} credential fields")

            # Check if fields are revealed (not password type)
            if 'type="password"' in content:
                self.log_validation("Credentials Workbook", False, "Password fields still present (not revealed)")
            else:
                self.log_validation("Credentials Workbook", True, "All credential fields revealed")

            return len(found_fields) > 0 and 'type="password"' not in content

        except Exception as e:
            self.log_validation("Credentials Workbook", False, f"Validation failed: {str(e)}")
            return False
